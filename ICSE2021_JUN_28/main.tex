...
bits for each leakage site. With the Central Limit Theorem, 
we can also provide an error bound for these estimation.
  
We have implemented the technique in a tool called \tool{}, which can
not only find very fine-grained side-channel vulnerabilities but can also estimate how many
...
exploit in practice and can be de-prioritized by developers.
