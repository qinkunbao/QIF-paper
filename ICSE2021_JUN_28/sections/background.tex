...
program accesses different memory addresses if the program \cready{has}{takes} different
\cready{sensitive inputs}{secret data}. As shown in Figure~\ref{fig:secret:cf} and Figure~\ref{fig:secret:da}, 
if a program shows different patterns in
control transfers or data accesses when the program processes different
sensitive inputs, the program is vulnerable to side-channel attacks.
...
or other \cready{}{more fine-grained} granularities\cready{}{~\cite{yarom2017cachebleed,moghimi2020copycat}}. 
...
can access the source code \cready{}{or the binary executable} of the target program.
