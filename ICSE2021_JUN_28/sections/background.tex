...
So, we obtai  $-\log(\frac{1}{256}) = 8$ bits of information, which is exactly the size
of a char variable.
...
\end{figure}
In this paper, we assume an attacker can use side channels to observe the secret-dependent control-flows in 
Figure~\ref{fig:side-channel}.
Therefore, an attacker can obtain two different observations for each leak site
depending on the value of the $\mathit{key}$: $A$ when function \textsf{foo} is executed, 
$\neg A$ when function \textsf{foo} is not executed, $B$ when function \textsf{bar} is
executed, and $\neg B$ when function \textsf{bar} is not executed. The
question is how much information can be leaked from this code if an
...
quantify the information leakage from a practical point of view. \jl{Where do we show this?}
}
...
We only consider only deterministic programs and assume an attacker 
has access to the source code \cready{}{or binary executable} of the target program.
