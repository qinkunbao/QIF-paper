...
instruction that might be affected by the input key. \cready{}{\tool{} works on one single path at a time. The symbolic execution engine is conceptually similar to the trace-based executor (e.g., SAGE~\cite{godefroid2008automated}). That is, we use symbolic execution to track secrets. When secrets are loaded into the memory, Abacus starts to interpret instructions symbolically. We treat secrets as symbols ($S$). For other variables, we use concrete values ($C$) 
from the execution. We do not know which instruction 
may manipulate a secret until we execute it. For each instruction, if all its operands and implicit memory accesses are all concrete values, then we do use the concrete calculation according to the semantics' of the instruction. Otherwise, we do the symbolic execution.}
