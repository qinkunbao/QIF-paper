...
We run the target binary under the dynamic binary instrumentation (DBI) tool
to record execution traces and the runtime information.
Once \cready{sensitive information}{secret data} is loaded into memory, we start to collect the trace. \cready{}{In this step, we also mark variables and buffers that are the secret data. It can be done either by adding an annotation on the source code (\textsf{make\_abacus\_symbolic}) or telling DBI tool the memory address and the length of secrets.}
...
instruction that might be affected by the input \cready{key}{secret data}. \cready{}{\tool{} works on one single path at a time. The memory model is conceptually similar to other offline executors (e.g., SAGE~\cite{godefroid2008automated}, the trace-based executor of BitBlaze~\cite{song2008bitblaze}). That is, we use symbolic execution to track secrets. When secrets are loaded into the memory, Abacus starts to interpret instructions symbolically. We treat secrets as symbols ($S$). For other variables, we use concrete values ($C$) 
from the execution. We do not know which instruction may manipulate a secret until we execute it. For each instruction, if all its operands and implicit memory accesses are concrete values, we do the concrete calculation and update the destination with the concrete value according to the semantics' of the instruction. Otherwise, we symbolically interpret the instruction and update the destination with a math formula.}
\subsubsection{Leakage estimation} We transfer the information leakage 
