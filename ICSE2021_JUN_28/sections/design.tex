...
Once \cready{sensitive information}{secret data} is loaded into memory, we start to collect the trace. \cready{}{In this step, we also tell Abacus which variables are the secret. It can be done either by adding an annotation on the source code or telling DBI tool the memory address and the length of secrets.}
...
may manipulate a secret until we execute it.}
