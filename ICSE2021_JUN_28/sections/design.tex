...
instruction that might be affected by the input \cready{key}{secret data}. \cready{}{\tool{} works on one single path at a time. The memory model is conceptually similar to the offline executors (e.g., SAGE~\cite{godefroid2008automated}, the trace-based executor of BitBlaze~\cite{song2008bitblaze}). That is, we use symbolic execution to track secrets. When secrets are loaded into the memory, Abacus starts to interpret instructions symbolically. We treat secrets as symbols ($S$). For other variables, we use concrete values ($C$) 
