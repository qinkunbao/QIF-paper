...
\tool{} not only discovers all the secret-dependent data accesses vulnerabilities reported 
by CacheD~\cite{203878}, but also finds many new ones. CacheD fails to detect some
vulnerabilities for two
...
Second, according to the CacheD paper, CacheD times out after 20 hours to process the
Libgcrypt 1.6.1\@. Therefore, CacheD applies some domain knowledge to simplify 
the analysis. 
While those optimizations do not introduce false positives, they may miss some
vulnerabilities. 
We notice that the number of instructions are different due to the different starting 
points and building options. 
