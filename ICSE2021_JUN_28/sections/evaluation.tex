...
\begin{table*}[h]
    ...
    \caption{Evaluation results overview. We evaluate two different versions of mbedTLS, five different
...
%    \resizebox{1.9\columnwidth}{!}{
    ...
    \begin{tabular}{llrrrrrrr}
        ...
        ECDSA              & mbedTLS 2.5              & 6                      &    6                & 0                    &  4,214,946   & 12.0  &  48 ~~  &   31      ~~  \\ 
        ECDSA              & mbedTLS 2.15             &   4                   &   4                 & 0                    &  4,192,558  & 11.7  & 102 ~~  & 1639     ~~         \\   
        ECDSA              & OpenSSL 1.0.2f           &  5                    & 4                   & 1                    &  8,248,322   & 8.8    & 101 ~~    & 62 ~~           \\   
        ECDSA              & OpenSSL 1.0.2k           &  5                    & 4                   & 1                    &  8,263,599    &  8.4  & 100 ~~     & 58 ~~            \\   
        ECDSA              & OpenSSL 1.1.0f           &  5                    & 4                   & 1                    &  6,100,465     &   8.4   & 76 ~~     & 42 ~~                  \\  
        ECDSA              & OpenSSL 1.1.1            & 0                      &  0                   &    0               &  10,244,076    &  0    &  121 ~~    & 0 ~~                 \\  
        ECDSA              & OpenSSL 1.1.1g           & 0                      &  0                   &  0                 &  9,266,191       & 0      & 102 ~~     & 59 ~~        \\  
        ...
        RSA                & OpenSSL 1.1.1            & 4                      &  4                  & 0                    & 18,207,016  & 1.0  & 8 ~~      & 53 ~~           \\
        RSA                & OpenSSL 1.1.1g           & 8                      &  8                  & 0                    & 18,536,796  & 10.0  & 5 ~~      & 780 ~~           \\
...
private keys as secrets. We use test cases of those libraries as executables.


We build the source code into 32-bit x86 Linux executables with GCC 8.0
under Ubuntu 14.04. After secrets are loaded in the memory, 
the pin tool start to collect the traces of each executable.
Although we use use symbol information to track back leakage
...
883 leakages in total from real-world crypto libraries. Among those 896
...
Every leakage found during the evaluation belong to the type of
...
lookups. mbedTLS 2.15 and 2.5 have the same implementation of both DES and AES, so
they have the same leakage report. One proper fix would be a scalar bit-sliced
implementation. We do not see the bit-sliced implementation of AES and
DES in various versions of mbedTLS\@. However, we find the new
...
estimating the number of keys that satisfy the constraints. During the evaluation,
...
side-channel leakages as well. In this section, we compare \tool{} with the
...
(e.g., the max leakage of AES in OpenSSL 1.1.1 is 4.4 bits, but other versions
...
table with 8-bit entries. In other words, our tools shows that lookup tables with smaller 
entries will leak less amount of information. Our tool suggests a smaller lookup
table can mitigate side-channel vulnerabilities. For example, Shown in Appendix~\ref{appendix:mbedtls_aes},
...
Even for the up-to-date version of the library, \tool{} still finds several
side-channel leakages. The vulnerability is severe by our tool.
Details of the leak can be found in Appendix \S\ref{unknown:leak} and 
Table \ref{tab:RSAOpenSSL1.1.1g}.
