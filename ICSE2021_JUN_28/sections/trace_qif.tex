...
and $ M $ \cready{is encrypted messages.}{represents the set consisting of all plaintext messages to be encrypted.} In a real execution, an adversary may have 
...
Next we compare the Eq.~(\ref{eq:2}) with the above \cready{slogan}{formulation}, we find $H(K)$
