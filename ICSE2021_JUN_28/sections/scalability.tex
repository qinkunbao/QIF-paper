...
%\vspace*{2pt}
\textbf{Our Solution:}
We abandon the IR design and take the \cready{engineer efforts}{engineering efforts} to implement 
...
Existing \cready{approach relies}{approaches rely} on satisfiability modulo theories (SMT) solvers 
...
A simple method is to \cready{pick elements $k$}{pick $k$ elements} from $K$ and check if the
\cready{element also contained}{element is also contained} in $K^o$. Assume $q$ elements satisfy this condition. In
...
we can only generate random input numbers for the \cready{rest input variables}{rest of the input variables} in
constraint $g_{i}$. With this simple yet effective trick, we can reduce the number of input
while still \cready{ensure}{ensuring} the accuracy.
...
observed value $\hat{p}$ \cready{to the calculate}{to calculate} the standard deviation, we can claim that
