...
\begin{enumerate}
    ...
    \item  \textbf{Quantifying side-channel leakages.}
          ...
          (\S\ref{eval:sym}, \S\ref{eval:asym}, and \S\ref{sec:eval_countermeasures})
\end{enumerate}
...
We also evaluate \tool{} on RSA. First, we find developers are more interested in 
fixing side-channels from RSA implementations. Second, those vulnerabilities
can leak more information are more likely to be fixed in the newer version.
...
same amount of leakage. We check the changelog and find only one code change for
patching vulnerabilities for RSA (CVE-2016-0702). 
...
which proves the effectiveness of our tool.
